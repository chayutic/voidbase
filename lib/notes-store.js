...
// ═══════════════════════════════════════════════════════════════
//
//  Notes are plain .md files in NOTES_DIR. No database, no index, no
//  frontmatter — the directory is the source of truth and stays readable
//  by any editor.
//
//  Filenames are `YYYYMMDD-HHmmss.md` and never change. The display
//  title is derived from the body at read time, so retitling a note is
//  just editing its first line.
 ...
 * A heading is a deliberate act; an opening line of "call the bank" is
 * not a title. Returns null when the note is untitled — the client
 * renders the timestamp instead.
 */
...
/**
 * First meaningful line that is not the title heading. Always shown in
 * the sidebar — without it, untitled notes are a wall of near-identical
 * timestamps.
 */
 ...
 *
 * Deliberately naive: Thai has no inter-word spaces, so word-boundary
 * matching would fail on it while plain substring matching works. At a
 * few hundred notes this scans the whole corpus in single-digit ms.
 */
