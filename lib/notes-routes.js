...
// ═══════════════════════════════════════════════════════════════
//
//  Mounted at /notes so a single Cloudflare Access application can
//  cover the page and its API together. Nothing here does auth — that
//  happens at the edge.
//
...
router.get("/", (req, res) => {
  res.sendFile(path.join(PUBLIC_DIR, "notes.html"), err => {
    // The page arrives in phase 2; until then say so plainly rather
    // than surfacing a raw ENOENT.
    if (err) res.status(404).type("text/plain").send("Notes UI not built yet.");
  });
});
